...
export function transpileNgxCore() {
  getProgram([path.resolve(ROOT, 'src/ionic-msal-native/core/index.ts')]).emit({
...
// remove reference to ionic-msal-native/core decorators
...
function removeIonicNativeDecorators(node: any) {
  if (node.decorators && node.decorators.length) {
    node.decorators = node.decorators.filter(
      (d: { expression: { module: string } }) => d.expression.module !== 'ionic-msal-native/core'
    );
  }
