...
function transformImports(file: ts.SourceFile, ctx: ts.TransformationContext, ngcBuild?: boolean) {
  ...
  // find the ionic-msal-native/core import statement
  const importStatement = (file.statements as any).find((s: any) => {
    return s.kind === ts.SyntaxKind.ImportDeclaration && s.moduleSpecifier.text === 'ionic-msal-native/core';
  });

  // we're only interested in files containing ionic-msal-native/core import statement
